...
| **Bus** | Carries message buffers between endpoints, validates device numbers, handles discovery or hotplug when needed, and defines memory-sharing and address rules such as how `SET_VQUEUE` addresses are interpreted. |
...
still has exactly one authoritative transport-message path documented by the
bus. Out-of-band signaling is a wakeup hint, not a second semantic notification.
